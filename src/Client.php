...
class Client
{
    ...
    public function textMessage($text, Closure $action)
    {
        return $this->on(self::getTextMessageEvent($action), self::getTextMessageChecker($text));
    }

    public function anyTextMessage(Closure $action)
    {
        return $this->on(self::getTextMessageEvent($action), self::getAnyTextMessageChecker());
    }
    ...
    protected static function getTextMessageEvent(Closure $action)
    {
        return function (Update $update) use ($action) {
    ...
    protected static function getTextMessageChecker($text)
    {
        return function (Update $update) use ($text) {
            $message = $update->getMessage();
            if (!is_null($message) && !is_null($message_text = $message->getText())) {
    ...
    protected static function getAnyTextMessageChecker()
    {
        return function (Update $update) {
            $message = $update->getMessage();
            return !is_null($message) && !is_null($message->getText());
        };
    }
    ...
    protected function getWasAddedToAGroupChecker()
    {
        return function (Update $update) {
            $message = $update->getMessage();
            if (!is_null($message) && !is_null($newChatMember = $message->getNewChatMember())) {
    ...
    protected function getWasRemovedFromAGroupChecker()
    {
        return function (Update $update) {
            $message = $update->getMessage();
            if (!is_null($message) && !is_null($leftChatMember = $message->getLeftChatMember())) {
